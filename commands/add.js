...
module.exports = {
    ...
    run(message, args) {
        ...
        if (args.length < 3) { channel.send(embeds.error("Invalid number of arguments. The correct usage is `.r add <hour:minute><am|pm> <month/day/year> <message>`, or leave `year` blank to use the current year.")); return; }
        ...
        const reminder = {
            date: new Date(date[2], date[0] - 1, date[1], time[0], time[1]).getTime(),
